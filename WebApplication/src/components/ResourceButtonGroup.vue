...
<template>
    <div class="d-grid d-lg-block gap-2">
        ...
        <Modal v-if="uiOptions.EnableDeleteResources" :id="'delete-modal-' + this.resourceOrthancId"
            :headerText="'Delete ' + `${$t(`${this.resourceLevel}`).toLocaleLowerCase()} `+ ' ?'" :okText="'Delete'"
            :cancelText="'Cancel'"
            :bodyText="'Are you sure you want to delete this ' + `${$t(`${this.resourceLevel}`)} `+ ' ?<br/>  This action can not be undone !'"
        ...
        <button v-if="hasSendTo" class="dropdown btn btn-sm btn-secondary m-1 dropdown-toggle" type="button"
            id="dropdownMenuId" data-bs-toggle="dropdown" aria-expanded="false">
