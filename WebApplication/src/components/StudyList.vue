...
export default {
  ...
  methods: {
    ...
    _updateFilter(dicomTagName, value) {
      ...
      this.$store.dispatch("studies/updateFilter", {
    ...
    async updateFilterFromRoute(filters) {
      ...
      if (filters === undefined) {
        ...
        console.log(
          "updateFilterFromRoute ... I think this code is not used anymore ..."
        ); // filters=={} when default url
      } else {
    ...
    async search() {
      if (this.isSearching) {
        ...
        await this.$store.dispatch("studies/updateFilterNoReload", {
          ...
          value: this.filterStudyDate,
        });
        await this.$store.dispatch("studies/updateFilterNoReload", {
          dicomTagName: "AccessionNumber",
          value: this.filterAccessionNumber,
        });
        await this.$store.dispatch("studies/updateFilterNoReload", {
          dicomTagName: "PatientID",
          value: this.filterPatientID,
        });
        await this.$store.dispatch("studies/updateFilterNoReload", {
          dicomTagName: "PatientName",
          value: this.filterPatientName,
        });
        await this.$store.dispatch("studies/updateFilterNoReload", {
          dicomTagName: "PatientBirthDate",
          value: this.filterPatientBirthDate,
        });
        await this.$store.dispatch("studies/updateFilterNoReload", {
          dicomTagName: "StudyDescription",
          value: this.filterStudyDescription,
        });
        await this.$store.dispatch("studies/reloadFilteredStudies");
        this.updateUrl();
      }
...
<template>
  <div>
    <table class="table table-responsive table-sm study-table">
      <thead>
        ...
        <th
          ...
          v-bind:class="
            'study-table-header cut-text ' + columns[columnTag].extraClasses
          "
        >
      ...
      <thead class="study-filter" v-on:keyup.enter="search">
        <th scope="col" class="px-2">
          <button
            @click="clearFilters"
            type="button"
            class="form-control study-list-filter btn filter-button"
            data-bs-toggle="tooltip"
            title="Clear filter"
          >
        ...
        <th v-for="columnTag in uiOptions.StudyListColumns" :key="columnTag">
          <input
            ...
            v-bind:class="getFilterClass('StudyDate')"
          />
          <input
          ...
          <input
            ...
            placeholder="1234"
            v-bind:class="getFilterClass('PatientID')"
          />
          <input
    ...
    <div
      v-if="!isSearching && notShowingAllResults"
      class="alert alert-danger bottom-fixed-alert"
      role="alert"
    >
      <i class="bi bi-exclamation-triangle-fill"></i> Not showing all results.
      You should refine your search criteria !
    </div>
    <div
      v-else-if="
        !isSearching &&
      ...
      "
      class="alert alert-warning bottom-fixed-alert"
      role="alert"
    >
      <i class="bi bi-exclamation-triangle-fill"></i> Enter a search criteria to
      show results !
    </div>
    <div
    ...
    <div
      ...
      class="alert alert-secondary bottom-fixed-alert"
      role="alert"
    >
      <span
        v-if="isSearching"
